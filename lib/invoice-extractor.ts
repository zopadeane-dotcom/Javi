...
export function extractFromText(rawText: string): InvoiceData {
  ...
  const numPatterns = [
    ...
    // "Nro.", "Núm.", "No.", "N°" — separador obligatorio para no confundir con "Número"
    /\b(?:nro|n[uú]m|n[°o])\b[.:\s\-]+([A-Z0-9][\w\-\/\.]{0,20})/i,
  ...
  function isValidSupplier(name: string): boolean {
    ...
    // Rechazar títulos personales SOLO si no hay forma jurídica (MR. CHAVA CAFÉS SL es empresa)
    if (/^(Mr\.|Mrs\.|Sr\.|Sra\.|Dr\.|Miss\s|Don\s|Doña\s)/i.test(name) && !new RegExp(LEGAL_FORMS, "i").test(name)) return false
