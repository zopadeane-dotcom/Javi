          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1 md:col-span-2">
              ...
              <div className="flex gap-2">
                <Select onValueChange={(v) => { if (v) setValue("supplier_id", v as string) }}>
            ...
            {/* Preview cálculo */}
            {baseAmount > 0 && (
              <div className="md:col-span-2 rounded-lg bg-muted p-3 text-sm space-y-1">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">IVA ({vatRate}%)</span>
                  <span>{new Intl.NumberFormat("es-ES", { style: "currency", currency: "EUR" }).format(preview.vat)}</span>
                </div>
                <div className="flex justify-between font-semibold">
                  <span>Total</span>
                  <span>{new Intl.NumberFormat("es-ES", { style: "currency", currency: "EUR" }).format(preview.total)}</span>
                </div>
              </div>
            )}
            ...
            <div className="flex items-center gap-2 md:col-span-2">
              <Checkbox
                id="is_deductible"
                defaultChecked
                onCheckedChange={(v) => setValue("is_deductible", !!v)}
              />
              <Label htmlFor="is_deductible">IVA deducible (para Modelo 303)</Label>
            </div>

            <div className="space-y-1 md:col-span-2">
              <Label htmlFor="file">Adjuntar factura (PDF o imagen)</Label>
